...
- If `.mcp.json` exists in CWD:
  ...
  - If no flag → "Found existing .mcp.json. Update it? [Y/n]"
    ...
    - N → skip MCP setup entirely (do not write `.mcp.json`), go directly to Step 8 (Permission setup)
...
|-------------|-----------|----------------------|-------------|
| youtrack | HTTP | `https://<INSTANCE>.youtrack.cloud/mcp` | Bearer token, written directly into the `Authorization` header value in `.mcp.json` (not an OS env var) |
| github | HTTP | `https://api.githubcopilot.com/mcp/` | Bearer token, written directly into the `Authorization` header value in `.mcp.json` (not an OS env var) |
...
5. **Validate file size** (guards against truncated downloads, error pages, or missing platform assets):
   ```bash
...
8. **Windows download fallback sequence:** If curl download failed AND platform is Windows (`MINGW*`/`MSYS*`):
   a. **PowerShell fallback (try first):** curl via Bash is unreliable on Windows — use PowerShell's `Invoke-WebRequest` which does not have this limitation:
      ...
      ```
      - After extraction, run size validation (step 5) against `$env:USERPROFILE\.claude\bin\gitea-mcp.exe`
...
9. **Non-Windows download fallback (Linux/macOS):** If curl download failed AND platform is NOT Windows:
    a. **wget fallback:** Check if wget is available: `command -v wget`
       - **If available:**
         ...
         ```
         - Run size validation (step 5) against `~/.claude/bin/{binary_name}`
...
**Post-write path validation (MANDATORY, gitea only):** After writing `.mcp.json`, if a `gitea` server entry is present, check its `"command"` value:
1. If `command` is still the bare binary name (`gitea-mcp` or `gitea-mcp.exe` — no path separator) rather than an absolute path → the Step 5→Step 6 path substitution was skipped or incomplete.
   - Display: `"ERROR: .mcp.json 'gitea' server command ('{command}') is not an absolute path. This only works if gitea-mcp happens to be on PATH. Returning to path collection."`
   - Return to Step 5 gitea section, re-resolve or re-collect the absolute path, then rewrite the `gitea` server entry in `.mcp.json`.
2. If `command` is an absolute path → proceed. (Do not apply this check to `redmine` or the YouTrack `npx` fallback — their bare commands are correct by design, per Step 6 above.)
...
**Restart caveat:** Claude Code reads `.mcp.json` once, at session start. Any server written or changed by Step 2/Step 6 in *this* run is NOT yet loaded into the current session — the connectivity check below will typically report `mcp_available: false` with "no matching MCP tool found" for those servers even when the configuration is correct. This is expected, not a real failure; do not treat it as `[FAIL]`. Keep track of which server names were newly added or modified in this run (vs. pre-existing servers preserved unchanged during `--update`) so step 2 below can message the two cases differently.
...
2. **Connectivity check:** Follow `../../core/mcp-detection.md` (with `check_write: false`):
   ...
   - If `mcp_available: false` AND `{server_name}` was newly added or changed in this run → `"[INFO] {server_name} configured — not yet loaded in this session. Restart Claude Code, then run /agent-flow:check-setup to verify connectivity."` (informational, not a failure — do not suggest token/URL troubleshooting for a server that has never been loaded)
   - If `mcp_available: false` AND `{server_name}` was already configured and unchanged before this run (e.g. preserved during `--update`) → `"[FAIL] {server_name}: {error}. Check your token and URL."` (If `error_type` is `"tls"`, append: `" Try adding NODE_OPTIONS: --use-system-ca to the env block in .mcp.json."`)
