...
| Draft skips e2e_test because "all remaining steps are optional given project config" | Check `.agent-flow/config.toml` explicitly via `../../../core/config-reader.md`. If absent, write `e2e_test.status = "skipped"` — DO NOT leave at "pending". |
