...
3. Parse **optional sections** — missing section → use defaults:
   ...
   - `### Notifications` → `notifications.webhook_url`, `notifications.on_events` (default: none; valid events: `pr-created`, `issue-blocked`, `pipeline-started`, `step-completed`, `pipeline-completed`, `pipeline-paused`, `pipeline-resumed` — the latter two are emitted by fix-bugs' clarification flow when a pipeline pauses on NEEDS_CLARIFICATION and resumes)
   ...
   - `### Pause Limits` → `pause_limits.pause_timeout` (default: `30 days`) — how long a `paused` pipeline (awaiting NEEDS_CLARIFICATION) is retained before autopilot treats it as abandoned. Valid range: 1 hour–365 days; values outside this range or otherwise malformed fall back to the default with a `[WARN]` log rather than blocking the pipeline. Consumed by `skills/autopilot/SKILL.md`'s `parse_pause_timeout()`.
