...
Method: each of the 10 core pipeline agents (`analyst`, `architect`, `browser-agent`, `fixer`, `deployment-verifier`, `reviewer`, `spec-analyst`, `spec-reviewer`, `spec-writer`, `test-engineer`) was independently reviewed by 2 adversarial subagents (structural/contract-craft lens + adversarial-reliability lens, with live research against current Claude Code subagent best practices) and reconciled by a third orchestrator agent before any edit was applied; every edit was verified against `tests/scenarios/step-completion-invariants-completeness.sh` and each agent's shape test where one exists.
